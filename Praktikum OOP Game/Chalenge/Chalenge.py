...
class BarangElektronik(ABC):
    ...
    # Getter untuk mengambil nilai stok (Read-only access)
    def get_stok(self):
        return self.__stok

    # Getter untuk harga dasar (diperlukan karena attributenya private)
    def get_harga_dasar(self):
        return self.__harga_dasar

    # Method untuk mengubah stok dengan validasi (Setter Logic)
...
# 2. INHERITANCE (Pewarisan) - Class Anak 1
...
# 3. INHERITANCE (Pewarisan) - Class Anak 2
...
# FITUR KERANJANG BELANJA (Fungsi Luar)
...
# ==========================================
# MAIN PROGRAM (Skenario User Story)
# ==========================================
if __name__ == "__main__":
    print("--- SETUP DATA ---")
    
    # 1) Admin membuat data produk (Stok awal 0 dulu agar sesuai skenario update)
    laptop_rog = Laptop("ROG Zephyrus", 0, 20000000, "Ryzen 9")
    hp_iphone = Smartphone("iPhone 13", 0, 15000000, "12MP")

    # 2) Admin update stok (Skenario Validasi)
    laptop_rog.tambah_stok(10)      # Berhasil
    hp_iphone.tambah_stok(-5)       # Gagal (Testing Encapsulation)
    hp_iphone.tambah_stok(20)       # Berhasil

    # 3) User membeli barang
    # Kita buat list dictionary agar bisa menampung Objek dan Jumlah yang dibeli
    keranjang_belanja = [
